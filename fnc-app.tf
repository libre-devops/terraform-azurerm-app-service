resource "azurerm_function_app" "function_app" {
  ...
  dynamic "auth_settings" {
    for_each = var.enable_auth_settings
    content {
      enabled                        = var.enable_auth_settings
      default_provider               = var.default_auth_provider
      allowed_external_redirect_urls = []
      issuer                         = format("https://sts.windows.net/%s/", var.tenant_id)
      ...
      dynamic "active_directory" {
        ...
        content {
          ...
          allowed_audiences = concat(formatlist("https://%s", [
            format("%s.azurewebsites.net", var.app_name)
          ]), [])
        }
      }
    }
  ...
  dynamic "backup" {
    ...
    content {
      ...
      storage_account_url = format("https://${var.storage_account_name}.blob.core.windows.net/${var.storage_container_name}%s", var.storage_account_name, var.storage_container_name)
      schedule {
